...
class CSV_Mammo_Cancer_Survival_All_Images_Dataset(Abstract_Onco_Dataset):
    ...
    def create_dataset(self, split_group, img_dir):
        ...
        for _row in self.metadata_json:
            ...
            visit_id, exam_id, split  = row['visit_id'], row['exam_id'], row['split_group']
            view = "{} {}".format(row['laterality'], row['view'])
            accession = "{}\t{}".format(visit_id, exam_id)
            file = row['file_path']

            dict_dataset[visit_id]['split'] = split
            dict_dataset[visit_id]['pid'] = visit_id
            #dict_dataset['years_to_cancer'] = 10
            #dict_dataset['years_to_last_followup'] = 10
            # Assign values to the dictionary for each patient

            # Check if 'years_to_cancer' exists in the row, if not, set it to 10
            if 'years_to_cancer' not in row:
                row['years_to_cancer'] = 1
            if 'years_to_last_followup' not in row:
                row['years_to_last_followup'] = 1    

            dict_dataset[visit_id]['years_to_cancer'] = row['years_to_cancer']
            dict_dataset[visit_id]['years_to_last_followup'] = row['years_to_last_followup']

            if 'exams' not in dict_dataset[visit_id]:
                dict_dataset[visit_id]['exams'] = {}
            if accession not in dict_dataset[visit_id]['exams']:
                dict_dataset[visit_id]['exams'][accession] = {
            ...
            dict_dataset[visit_id]['exams'][accession]['views'].append(view)
            dict_dataset[visit_id]['exams'][accession]['files'].append(file)
